...
def GenMolRep(ifile,optim,resid,charge):
    ...
    if iform[1] == 'smi': 
        ...
        os.system('babel -i%s %s -omol %s.mol' % (iform[1], ifile, iform[0]))
    ...
    if iform[1] == 'pdb':
        os.system('/Users/Sriram/mcpro/miscexec/clu -t:f=pdb %s.pdb -r %s.z -n:f=p /tmp/clu.pdb -m ma' % (iform[0], resid))
    return(True)
...
def make_graphs(atoms, coos, bonds):
    ...
    for i in all_paths.keys():
        for j in all_paths[i].keys():
            if len(all_paths[i][j]) == 2:
                all_bonds.append(all_paths[i][j])
            elif len(all_paths[i][j]) == 3:
                new_angs.append(all_paths[i][j])
            elif len(all_paths[i][j]) == 4:
                new_tors.append(all_paths[i][j])
...
# PRINTING ACTUAL Z-MATRIX
    ...
    ofile.write('BOSS Z-Matrix with LSDautozmat\n')
